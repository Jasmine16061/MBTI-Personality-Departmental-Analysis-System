...
export default function App() {
  ...
  const handleLogin = (e) => {
    ...
    } else if (loginInput.toLowerCase() === 'professor') {
      setUserRole('professor');
      setCurrentUserId('professor');
      setActiveTab('course_feedback');
      showNotification('Logged in as Professor');
    } else {
      setUserRole('student');
      setCurrentUserId(loginInput.toUpperCase());
            ...
            <div className="flex items-center bg-black/20 p-2 border-2 border-white/20 w-full md:w-auto justify-end">
              {!userRole ? (
                <form onSubmit={handleLogin} className="flex items-center gap-2 w-full md:w-auto">
                    <UserCircle className="w-5 h-5 text-amber-200 shrink-0" />
                    <input 
                        type="text" 
                        value={loginInput}
                        onChange={(e) => setLoginInput(e.target.value)}
                        placeholder="Enter ID (Admin/Professor/StudentID)"
                        className="px-2 py-1 text-sm text-black font-bold outline-none w-full md:w-64"
                    />
                    <button type="submit" className="bg-white text-[#99000F] px-3 py-1 text-xs font-bold uppercase hover:bg-amber-100 shrink-0">
                        Login
                    </button>
                </form>
              ) : (
...
function PersonalProfile({ currentUserId, showNotification }) {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        if (isNewProfile) {
            // FIXED: If new profile, POST to create
            const res = await fetch(`${API_BASE_URL}/students`, {
                method: 'POST',
            ...
            // Existing profile, PUT to update
